...
def dashboard(request):
    ...
    # Set the figure size and margins for industry graph

    # Set the appropriate response data
    data = {
...
#         try:
#             graduation_dates = candidate.graduation_date.strip("[]").replace("'", "").split(", ")
#             if "Education Missing" in graduation_dates:
#                 continue
#
#             age = []
#             for date in graduation_dates:
#                 year = int(date[-4:])
#                 age.append(datetime.date.today().year - year)
#             ages.extend(age)
#         except ValueError:
#             pass
#
#     # Generate age distribution data
#     age_data = dict(Counter(ages))
#     age_labels = []
#     age_counts = []
#
...
#         count = sum(age_data.get(age, 0) for age in range(lower_bound, upper_bound))
#         age_labels.append(age_range)
#         age_counts.append(count)
#
#     # Generate the age distribution chart
